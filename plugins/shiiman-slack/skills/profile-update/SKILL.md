...
```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/profile-updater/slack_profile.py show
```

### 2. 表示名を変更

```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/profile-updater/slack_profile.py update --display-name "新しい表示名"
```

### 3. ステータスを設定

```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/profile-updater/slack_profile.py update \
  --status-text "会議中" \
...
```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/profile-updater/slack_profile.py clear-status
```
