...
```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/gmail-list-unread/google_gmail.py search --query "<query>"
```

### 最大件数を指定

```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/gmail-list-unread/google_gmail.py search --query "<query>" --max 50
```

### 本文プレビューを含める

```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/gmail-list-unread/google_gmail.py search --query "<query>" --include-body
```
