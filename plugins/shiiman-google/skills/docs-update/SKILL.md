...
```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/google_docs.py update --doc-id "ドキュメントID" --content "追加テキスト"
```

### 末尾に追加

```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/google_docs.py update --doc-id "ドキュメントID" --content "追加テキスト" --append
```
