...
```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/google_gmail.py starred
```

### 最大件数を指定

```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/google_gmail.py starred --max 50
```

### JSON 形式で出力

```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/google_gmail.py --format json starred
```
