...
```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/auth-login/google_auth.py profiles
```

### プロファイルを切り替え

```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/auth-login/google_auth.py switch <profile-name>
```

例:
```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/auth-login/google_auth.py switch work
python ${CLAUDE_PLUGIN_ROOT}/scripts/auth-login/google_auth.py switch personal
```
