...
```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/drive-list/google_drive.py search --query "mimeType='application/vnd.google-apps.form' and name contains '<検索キーワード>'"
```

### プロファイル指定で検索

```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/drive-list/google_drive.py search --profile <profile-name> --query "mimeType='application/vnd.google-apps.form' and name contains '<検索キーワード>'"
```
