---
name: model-switcher
description: タスクの複雑さに応じて Claude のモデルを自動的に切り替える。「モデルを切り替えて」「opus にして」「sonnet で」「高性能モデルで」「軽いモデルで」「コスト優先で」「品質優先で」などで起動。複雑なタスクでは opus を、単純なタスクでは sonnet を自動選択してコストと品質のバランスを最適化。
