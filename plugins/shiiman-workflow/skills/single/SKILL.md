...
description: Issue/PR なしで計画書からタスクを実行しコミットメッセージを出力する軽量フロー。「シングルフロー」「shiiman-workflow:single」「軽量フロー」「Issue なしフロー」「シンプル実行」「計画書実行」「コミットのみフロー」などで起動。PR を作らない軽量な開発フロー。
allowed-tools:
  [
    Read,
    Write,
...
```text
/shiiman-workflow:single - 軽量シングルエージェントフロー
...
### 代替手段
- `shiiman-workflow:single --plan` で新しい計画を作成
- `shiiman-workflow:single タスク説明` で直接実行
```
...
4. ユーザーが計画を承認したら、自動的に shiiman-workflow:single の実行フェーズに進む
